...
public class MainActivity extends AppCompatActivity
   ...
   // IMPLEMENTED MENU FOR THE CATEGORY ITEMS
   @Override
   public boolean onCreateOptionsMenu(Menu menu) {
      ...
      ArrayAdapter<CharSequence> adapter = ArrayAdapter.createFromResource(this, R.array.todolist_category_array, android.R.layout.simple_spinner_item);
   ...
   public void onItemSelected(AdapterView<?> parent, View view, int pos, long id) {
      ...
      if (selected.equalsIgnoreCase("All"))
      {
         ...
         {
         adapter.swapCursor(fetchDataForCategory(db, selected));
      }
   }
   ...
   public void closeDialog(int year, int month, int day, String description, String category) {
      addToDo(db, description, dateFormatter(year, month, day), category);
      cursor = getAllItems(db);
   ...
   private long addToDo(SQLiteDatabase db, String description, String duedate, String category) {
      ...
      // ADDED CATEGORY COLUMN FOR SQL INSERT DATABASE
      cv.put(Contract.TABLE_TODO.COLUMN_NAME_CATEGORY, category);
      return db.insert(Contract.TABLE_TODO.TABLE_NAME, null, cv);
   }

   public String dateFormatter(int year, int month, int day) {
   ...
   private int updateToDo(SQLiteDatabase db, int year, int month, int day, String description, String category, long id) {
      ...
      String duedate = dateFormatter(year, month - 1, day);

      ContentValues cv = new ContentValues();
      cv.put(Contract.TABLE_TODO.COLUMN_NAME_DESCRIPTION, description);
      cv.put(Contract.TABLE_TODO.COLUMN_NAME_DUE_DATE, duedate);

      // ADDED CATEGORY COLUMN FOR SQL UPDATE DATABASE
      cv.put(Contract.TABLE_TODO.COLUMN_NAME_CATEGORY, category);
   ...
   // FETCHING THE DATA FROM SQL DATABASE AS PER THE MENU'S ITEM
   private Cursor fetchDataForCategory(SQLiteDatabase db, String category) {
